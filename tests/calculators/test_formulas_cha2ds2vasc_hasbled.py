...
CHA2DS2-VASc: ICC(1), HAS(1), Idade>=75(2) / 65-74(1), DM(1), AVC/AIT/TEV prévio(2),
...
HAS-BLED: HAS não controlada(1), função renal alterada(1), função hepática alterada(1),
          AVC/AIT/TEV prévio(1), sangramento prévio(1), INR lábil(1), idade>65(1),
          álcool/drogas(1), medicamentos predisponentes(1). Máximo 9.
...
def test_todos_positivos_masculino_80a():
    inputs = dict(
        ...
        avc_avc_tev_previo=True, doenca_vascular=True,
...
def test_avc_previo_soma_nos_dois_escores():
    s = _scores(calculate({"idade": 50, "sexo": "M", "avc_avc_tev_previo": True}))
    assert s["chads_vasc"] == 2   # AVC prévio vale 2 no CHADS
    assert s["has_bled"] == 1     # e 1 no HAS-BLED
